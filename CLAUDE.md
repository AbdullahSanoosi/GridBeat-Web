...
**Done — Phase 2 (live timing), partial:** the full WS state machine
(`src/lib/live/store.ts`, a Zustand port of `live_timing_provider.dart`),
REST bootstrap, session-change detection, and three of the live page's tabs:
Tower (leaderboard + weather panel + fastest-lap overlay), Comms (race
control + pit stops + team radio playback), Map (Canvas track map with the
position-interpolation engine).

**Not started yet:**
- Driver/constructor detail pages (`/driver/[id]`, `/constructor/[id]`) —
  Phase 1 leftover, ports `driver_details_screen.dart`/
  `constructor_details_screen.dart` + `enrichment_provider.dart`.
- Race progression chart (`/stats/quali-to-race`) — ports
  `race_progression_screen.dart` + `race_progression_provider.dart`.
- Telemetry Compare (multi-driver speed/throttle/brake/gear/RPM charts) —
  ports `telemetry_compare.dart`. The store already collects
  `telemetryHistory`/`lapTimeHistory`/`currentLapSectors`, so the data layer
  is ready; this is UI-only work (Recharts, already installed).
- Client-side DVR/playback buffer (delay slider, pause, skip-to-live) —
  deliberately deferred in the initial live-timing pass; messages currently
  apply immediately in real time with no buffering. See the "Deliberately
  not ported" note atop `src/lib/live/store.ts`.
...
6. **This dev/testing environment doesn't visually composite frames** —
   ...
   catch, since this pane can silently fail to reproduce them.
